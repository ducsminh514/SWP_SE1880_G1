...
public class QuestionDAO extends DBContext implements GenericDAO<Question> {
    ...
    public List<Question> getQuestionByFilter(String search, String subject, String level, String status, int page, int pageSize) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                // Basic question properties that don't require additional queries
                ...
                question.setCreateTime(rs.getDate("CreateAt"));
                question.setUpdateTime(rs.getDate("UpdateAt"));
                question.setMp3(rs.getString("Mp3"));
                question.setLessonQuizId(rs.getInt("LessonQuizID"));

                // Store IDs for later batch loading - don't load entities here
                // We'll set the actual entity objects in the controller
                // We're just preparing a shell object here

                // Create temporary subject and questionType objects that only have IDs
                Subject tempSubject = new Subject();
                tempSubject.setSubjectId(rs.getInt("SubjectId"));
                question.setSubject(tempSubject);

                QuestionType tempQuestionType = new QuestionType();
                tempQuestionType.setQuestionTypeId(rs.getInt("QuestionTypeID"));
                question.setQuestionType(tempQuestionType);

                // Add to list - we'll populate related data later in bulk
                questions.add(question);
            }

            // Now that we have all questions, we can:
            // 1. Extract all unique subject IDs and question type IDs
            // 2. Load all subjects and question types in bulk
            // 3. Load all images in bulk
            // 4. Associate the loaded data with the questions

            // This is now done in the Controller's getQuestionsWithRelatedData method
    ...
    public ArrayList<Question> getQuestion(int quizId) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                c.setLessonQuizId(rs.getInt("LessonQuizId"));
                listQuestion.add(c);
            }
            return listQuestion;
        } catch (SQLException e) {
    ...
    // Add a new method to support bulk loading of subjects
    public Map<Integer, Subject> getSubjectsByIds(List<Integer> subjectIds) {
        ...
        String sql = "SELECT * FROM Subjects WHERE SubjectId IN (" + placeholders.toString() + ")";
        ...
        try {
            ...
            for (int i = 0; i < subjectIds.size(); i++) {
                st.setInt(i + 1, subjectIds.get(i));
            }
            ...
            while (rs.next()) {
                Subject subject = subjectDao.getFromResultSet(rs);
                subjectMap.put(subject.getSubjectId(), subject);
            }
        } catch (SQLException e) {
            System.out.println("Error getting subjects by IDs: " + e.getMessage());
        } finally {
            closeResources();
        }
